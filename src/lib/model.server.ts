...
function footballModel(
  ...
  const homeDefence = mean(
    home.map((r) => r.conceded),
    LEAGUE_AVG_GOALS,
  );
  ...
  const awayDefence = mean(
    away.map((r) => r.conceded),
    LEAGUE_AVG_GOALS,
  );

  // shrink towards league average when sample is small
  const shrink = (v: number, n: number) => (v * n + LEAGUE_AVG_GOALS * 3) / (n + 3);
  ...
  const markets: Market[] = [
    {
      ...
      selections: [sel("1x2:home", "Home Win", pHome), sel("1x2:away", "Away Win", 1 - pHome)],
    },
    {
      id: "spread",
      name: "Handicap",
